use std::fmt;
use crate::chess::{Color, Piece, Move, MoveType, ep_cap_square};
...
impl Board {
    ...
    pub fn empty() -> Board {
        Board { piece_boards: [BitBoard::EMPTY; 6], color_boards: [BitBoard::EMPTY; 2] }
    }
    #[inline]
    ...
    pub fn piece_at(&self, sq: Square) -> Option<Piece> {
        const PIECES: [Piece; 6] =
            [Piece::King, Piece::Queen, Piece::Bishop, Piece::Knight, Piece::Rook, Piece::Pawn];
        for p in PIECES {
    ...
    pub fn do_move(&mut self, m: Move) {
        let (us, them) = if self.color_boards[0].is_set(m.from) {(Color::White, Color::Black)}
                         else                                   {(Color::Black, Color::White)};
        let our_piece = match m.typ {
            MoveType::Promotion(p) | MoveType::PromotionCapture((p,_))
                => p,
            _   => m.piece
        };
        self.unset(m.from, us, m.piece);
        match m.typ {
            MoveType::Capture(p) | MoveType::PromotionCapture((_,p))
                => self.unset(m.to, them, p),
            MoveType::Enpassant => self.unset(ep_cap_square(m.to.file()).relative(them), them, Piece::Pawn),
    ...
    pub fn undo_move(&mut self, m: Move) {
        let (us, them) = if self.color_boards[0].is_set(m.to) {(Color::White, Color::Black)}
                         else                                 {(Color::Black, Color::White)};
        let our_piece = match m.typ {
            MoveType::Promotion(p) | MoveType::PromotionCapture((p,_))
                => p,
        ...
        match m.typ {
            MoveType::Capture(p) | MoveType::PromotionCapture((_,p))
                => self.set(m.to, them, p),
            MoveType::Enpassant => self.set(ep_cap_square(m.to.file()).relative(them), them, Piece::Pawn),
