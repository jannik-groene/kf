...
fn evaluate_pawns(pos: &mut Position, phase: i32) -> i32 {
    ...
    for pawn in pawns_us.iter() {
        ...
        res += 100 + piece_table_value(Piece::PAWN, pos.color(), pawn, phase);
        ...
        if in_front & (pawns_them | guarded_them | pawns_us) == 0 {
            res_early += match pos.color() {
                Color::WHITE => 30 / (7 - pawn.index() as i32 / 8),
                Color::BLACK => 30 / (pawn.index() as i32 / 8)
            };
            res_late += match pos.color() {
                Color::WHITE => 60 / (7 - pawn.index() as i32 / 8),
                Color::BLACK => 60 / (pawn.index() as i32 / 8)
            };
        }
...
fn bug_hunt() {
    ...
    let mut pos2 = Position::from_fen(String::from("r6r/ppp1nkbp/2nB2p1/4p3/4P3/6PP/qPPQNPB1/2KR3R w - - 0 15")).unwrap();
